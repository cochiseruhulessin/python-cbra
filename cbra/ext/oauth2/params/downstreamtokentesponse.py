...
async def get(
    ...
    async with (await get_client(clients, storage, client_id)) as provider:
        token = await provider.obtain(
            code=code,
            state=state,
            redirect_uri=f'{request.url.scheme}://{request.url.netloc}{request.url.path}'
        )
        await provider.verify(token, nonce=expected_nonce)
